...
evaluate_all.py
===============
Evaluate reconstruction quality for one or more generation experiment folders.

FOLDER STRUCTURE EXPECTED
--------------------------
Test images:
    test_dir/
        00001_aircraft_carrier.jpg   (or .png, etc.)
        00002_wok.jpg
        ...

Generated images (one folder per experiment):
    gen_dir/
        reconstructed_0000.png       (index 0 → matches sorted test image 0)
        reconstructed_0001.png
        ...

The pairing is by SORTED INDEX — the i-th reconstructed image corresponds
to the i-th test image when both are sorted alphabetically.
...
python evaluate_all.py \
    ...
    --gen_dirs  /path/to/exp_simple_caption \
    --names     simple_caption

# Evaluate multiple experiment folders in one run:
python evaluate_all.py \
    --test_dir  /path/to/test_images \
    --gen_dirs  /path/to/exp_notext /path/to/exp_simple_caption /path/to/exp_img2img /path/to/exp_img2img_caption \
    --names     notext simple_caption img2img img2img_caption

# Save results to CSV:
python evaluate_all.py --test_dir ... --gen_dirs ... --names ... --out results.csv
...
Perceptual (mid-level):
    LPIPS     ↓   Learned perceptual patch similarity (VGG)
    DreamSim  ↓   Holistic perceptual similarity (CLIP+OpenCLIP+DINO ensemble)
                  NeurIPS 2023 Spotlight — best human-alignment currently available

High-level:
    AlexNet2  ↑   AlexNet layer-2 cosine similarity
    AlexNet5  ↑   AlexNet layer-5 cosine similarity
    Inception ↑   Inception-v3 pool cosine similarity
    CLIP      ↑   CLIP ViT-L/14 cosine similarity
    SwAV      ↓   SwAV ResNet-50 cosine distance

INSTALL DEPENDENCIES
--------------------
    pip install dreamsim lpips openai-clip torch torchvision scipy scikit-image pillow
"""
