...
#   3. Generation       — SDXL + IP-Adapter, one image per top-N class
#
# Outputs per image:
#   image_XXXX/
#     retrieved_classes.json  — top-N classes + raw cosine scores
#     candidate_0.png ... candidate_N.png
...
def generate_candidates(h, retrieved_classes, generator_sdxl, gen, image_dir):
    for item in retrieved_classes:
        ...
        image.save(os.path.join(image_dir, f"candidate_{rank}.png"))
        print(f"    candidate_{rank}: '{prompt}'  cosine={item['raw_cosine']:.3f}")
...
def run_pipeline(eeg_embeds, img_features, class_names,
    ...
    for i in range(n):
        print(f"\n{'='*60}")
        print(f"Image {i:04d} / {n-1}  gt={class_names[i]}")
        print(f"{'='*60}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="EEG-to-Image generation: retrieve -> prior -> generate"
    )
