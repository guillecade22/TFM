...
#                              + saves CLIP embeddings of all candidates to disk
#   4. Weight Optimization   — gradient descent to find best W_RETRIEVAL / W_CANDIDATE
#                              using the saved embeddings (no re-generation needed)
#   5. Re-Ranking            — apply optimized weights and select best candidate
#
# The generation (stages 1-3) is run once. Weight optimization (stage 4) can be
# re-run cheaply any number of times using --opt_only.
...
# Weight optimization hyperparameters
OPT_LR        = 0.01   # learning rate
OPT_STEPS     = 500    # gradient descent steps
OPT_LOG_EVERY = 50     # print loss every N steps
...
def load_features():
    img_features = torch.load(VIT_H_14_FEATURES_TEST,
                               map_location=device)["img_features"]
...
# --- STAGE 3: CANDIDATE GENERATION + CLIP EMBEDDING EXTRACTION ---------------

def generate_candidates(h, retrieved_classes, generator_sdxl, gen,
                        image_dir, eeg_embed):
    """
    Generate one SDXL image per retrieved class.
    Extracts and saves the CLIP embedding of each candidate so that
    weight optimization runs without re-generating images.

    Saves per candidate:
        candidate_K.png
        candidate_K_clip.pt  — CLIP embedding [1, dim]

    Returns list of dicts: rank, class, raw_cosine, raw_candidate, image, path.
    """
    eeg_norm = F.normalize(eeg_embed.squeeze().unsqueeze(0), dim=-1)
    ...
    for item in retrieved_classes:
        ...
        img_path  = os.path.join(image_dir, f"candidate_{rank}.png")
        clip_path = os.path.join(image_dir, f"candidate_{rank}_clip.pt")
        image.save(img_path)

        clip_emb      = extract_clip_embedding(image)           # [1, dim]
        torch.save(clip_emb.cpu(), clip_path)

        raw_candidate = (clip_emb @ eeg_norm.T).item()          # candidate @ eeg

        candidates.append({
            "rank":          rank,
            "class":         item["class"],
            "raw_cosine":    item["raw_cosine"],  # eeg @ class_img (retrieval)
            "raw_candidate": raw_candidate,        # candidate_clip @ eeg (rerank)
            "image":         image,
            "path":          img_path,
        })
...
def load_all_scores(output_dir, class_names, top_n):
    """
    Load pre-computed retrieval and candidate scores for all images from disk.

    Returns:
        score_retrieval : Tensor [N, top_n]  — (raw_cosine_retrieval + 1) / 2
        score_candidate : Tensor [N, top_n]  — (raw_cosine_candidate + 1) / 2
        gt_indices      : Tensor [N]          — index of gt class in top-n (-1 if absent)
    """
    image_dirs = sorted([
    ...
    for image_dir in image_dirs:
        ...
        # Load scores saved in rerank_scores.json (written after first generation)
        ...
        if os.path.exists(rerank_path):
            ...
            for cand in rerank_data["candidates"]:
                score_map[cand["old_rank"]] = cand["scores"]["raw_candidate"]

        row_retrieval = []
        row_candidate = []
        for r in retrieved:
            row_retrieval.append((r["raw_cosine"]             + 1.0) / 2.0)
            row_candidate.append((score_map.get(r["rank"], 0) + 1.0) / 2.0)

        all_score_retrieval.append(row_retrieval)
        all_score_candidate.append(row_candidate)
        all_gt_indices.append(gt_idx)

    return (
        torch.tensor(all_score_retrieval, dtype=torch.float32),
        torch.tensor(all_score_candidate, dtype=torch.float32),
        torch.tensor(all_gt_indices,      dtype=torch.long),
    )


def optimize_weights(score_retrieval, score_candidate, gt_indices):
    """
    Find optimal W_RETRIEVAL and W_CANDIDATE via gradient descent.

    Objective: maximise the log-probability of the correct candidate (cross-entropy).

        final_score[i, k] = w_r * score_retrieval[i, k] + w_c * score_candidate[i, k]
        loss = cross_entropy(final_score, gt_indices)

    Weights are constrained to [0,1] and sum to 1 via softmax parameterisation:
        [w_r, w_c] = softmax([logit_r, logit_c])

    Only images where gt is in the top-N contribute to the loss.

    Returns:
        w_retrieval, w_candidate : optimised floats
        loss_history             : list of loss values
    """
    mask = gt_indices >= 0
    sr   = score_retrieval[mask].to(device)
    sc   = score_candidate[mask].to(device)
    ...
    # Softmax parameterisation ensures weights sum to 1 and stay in [0,1]
    ...
    for step in range(OPT_STEPS):
        ...
        if step % OPT_LOG_EVERY == 0 or step == OPT_STEPS - 1:
            ...
            print(f"  step {step:4d} | loss={loss.item():.4f} | "
                  ...
                  f"w_candidate={w_[1].item():.4f}")

    final = F.softmax(logits, dim=0).detach()
    w_r   = final[0].item()
    w_c   = final[1].item()
    print(f"\n  Optimized: w_retrieval={w_r:.4f}  w_candidate={w_c:.4f}")

    return w_r, w_c, loss_history


# --- STAGE 5: RE-RANKING WITH GIVEN WEIGHTS -----------------------------------

def rerank_candidates(candidates, eeg_embed, w_retrieval, w_candidate):
    eeg_norm = F.normalize(eeg_embed.squeeze().unsqueeze(0), dim=-1)
    ...
    for cand in candidates:
        ...
        raw_candidate   = cand.get("raw_candidate")
        if raw_candidate is None:
            clip_emb      = extract_clip_embedding(cand["image"])
            raw_candidate = (clip_emb @ eeg_norm.T).item()
        ...
        scored.append({
            ...
            "scores": {
                ...
                "w_candidate":     round(w_candidate,        6),
...
def run_pipeline(eeg_embeds, img_features, class_names,
    ...
    # ------------------------------------------------------------------
    # PHASE 1: Generate all candidates and save CLIP embeddings
    # ------------------------------------------------------------------
    print(f"\n{'='*60}")
    ...
    for i in range(n):
        ...
        candidates = generate_candidates(
            h, retrieved, generator_sdxl, gen, image_dir, eeg_embeds[i]
        )
        all_candidates.append(candidates)

        # Save initial rerank_scores.json with raw scores so load_all_scores
        # can read raw_candidate values without needing the images in memory
        gt_class = class_names[i]
        initial_scored = []
        for cand in candidates:
            initial_scored.append({
                "old_rank":       cand["rank"],
                "new_rank":       cand["rank"],   # placeholder, updated in phase 3
                "class":          cand["class"],
                "candidate_path": cand["path"],
                "scores": {
                    "raw_retrieval":   round(cand["raw_cosine"],    6),
                    "raw_candidate":   round(cand["raw_candidate"], 6),
                    "score_retrieval": round((cand["raw_cosine"]    + 1) / 2, 6),
                    "score_candidate": round((cand["raw_candidate"] + 1) / 2, 6),
                    "w_retrieval":     W_RETRIEVAL,
                    "w_candidate":     W_CANDIDATE,
                    "final_score":     0.0,   # filled in phase 3
                },
            })
    ...
    score_retrieval, score_candidate, gt_indices = load_all_scores(
        output_dir, class_names, TOP_N
    )
    w_retrieval, w_candidate, loss_history = optimize_weights(
        score_retrieval, score_candidate, gt_indices
    )
    ...
    for i in range(n):
        ...
        scored, best = rerank_candidates(
            candidates, eeg_embeds[i], w_retrieval, w_candidate
        )
