...
def evaluate_model(sub, eeg_model, dataloader, device, text_features_all, img_features_all, k, config):
    ...
    # -------- NEW: storage for global contrastive loss --------
    all_eeg_features = []
    all_img_features = []
    all_text_features = []
    ...
    with torch.no_grad():
        for batch_idx, (eeg_data, labels, text, text_features, img, img_features) in enumerate(dataloader):
            ...
            # -------- STORE FEATURES FOR GLOBAL LOSS --------
    ...
    img_loss = eeg_model.loss_func(all_eeg_features, all_img_features, logit_scale)
    text_loss = eeg_model.loss_func(all_eeg_features, all_text_features, logit_scale)

    total_loss = alpha * img_loss + (1 - alpha) * text_loss
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default="/hhome/ricse01/TFM/required/Preprocessed_data_250Hz/Preprocessed_data_250Hz/", help='Path to the EEG dataset')
    ...
    for sub in subjects:
        ...
        test_loader = DataLoader(test_dataset, batch_size=1, shuffle=True, num_workers=0, drop_last=False) #drop_last was True
